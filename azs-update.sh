...
show_help() {
    ...
    echo "  远程部署: $0 -t root:192.168.1.100:password -v 1.0.1"
    echo "  多目标部署: $0 -t local -t root:192.168.1.100:password -t admin:192.168.1.101:password -v 1.0.1"
    exit 0
}
...
deploy_to_target() {
    ...
    if [ "$target" = "local" ]; then
        ...
        sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$ip" "$(typeset -f docker_login run_docker_commands); run_docker_commands '$version'"
    fi
    return $?
}
